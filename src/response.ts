...
export function validateResponse(routeSpec, response, res, spec:any = {}) {
    ...
    validateValue('', response, routeSpec[res.req?.method?.toLowerCase()] || responseObj.content ? responseObj.content['application/json'] : responseObj, set(response), errors, spec.definitions, spec.parameters);
